...
## Prep tiers (so you don’t drown)

Use these tags mentally as you rehearse:

- **Must land**: the one sentence the audience should remember.
- **Show**: what the animation/slide is doing (so you narrate it, not compete with it).
- **Transition**: the single bridge sentence into the next idea.
- **Optional**: extra detail only if you have time or the audience asks.

---

## Rehearsal cards (10–12 min run-through)

Use this when you want a fast, coherent practice pass without rereading everything.

### Opening (Title → Timeline → Missing Waveforms)

- **Must land**: GR was “pen-and-paper” in 1915; today it’s a digital lab because we learned how to simulate it.
- **Show**: timeline = decades stuck (AI + NR) until math tricks + GPUs.
- **Must land**: LIGO needed **NR merger waveforms**; without them GW astronomy is blind.
- **Transition**: “So why did Newton’s gravity fail—and what did Einstein change?”

### Why GR exists (Newton → geometry heritage)

- **Must land**: Newton implies instant gravity; relativity forbids instant information.
- **Show**: Sun/Earth: Earth continues ~8.3 minutes after the Sun “vanishes”.
- **Must land**: Einstein’s leap was physical, but the math was inherited (Gauss→Riemann→Christoffel→Ricci/LC).
- **Transition**: “Now we can finally say what GR is: matter-energy on the right, geometry on the left.”

### The geometry ladder (Einstein eq → metric → curvature → motion)

- **Must land**: \(G_{\mu\nu}=8\pi T_{\mu\nu}\) hides **10 coupled, non-linear PDEs**.
- **Must land**: the **metric** \(g_{\mu\nu}\) is the lookup table that turns coordinate steps into physical \(ds\).
- **Show**: \(ds\) demo: far from mass both panels read \(ds\approx1\); only near mass does curved space stretch.
- **Must land**: Christoffels = “direction-corrections”; Riemann = curvature/tidal field; Ricci/Einstein = compressed curvature that enters the field equation.
- **Must land**: free fall is geodesics (straightest paths); black holes trap light because light cones tilt inward.
- **Transition**: “That’s the physics. Now: how does a computer actually solve these equations?”

### From equations to a movie (grid → 3+1 → ADM/BSSN/CCZ4)

- **Must land**: computers need a **grid** and an **initial-value problem** (now → next).
- **Must land**: **3+1 / ADM** rewrites the same GR into evolving 3D slices (lapse/shift control the coordinates).
- **Must land**: 10 equations become **6 evolution + 4 constraints**; computers leak constraint error unless we control it.
- **Must land**: ADM was unstable numerically → **BSSN** fixes variables → **CCZ4** damps constraint drift.
- **Transition**: “Once stability was solved, the big scientific payoff became possible.”

### Breakthrough + payoff (2005 → LIGO)

- **Must land**: 2005 was the turning point: stable binary black hole mergers.
- **Show**: inspiral → merger → ringdown waveform = what LIGO matches.
- **Transition**: “But we still have to trust the numerics—and run them fast.”

### Trust + hardware (convergence → GPUs → AMR)

- **Must land**: trust comes from **convergence**; 4th order → expect \(Q\to16\).
- **Must land**: NR is stencil/memory-bandwidth heavy → GPUs + HBM win.
- **Must land**: **AMR** = high resolution only where curvature is large.
- **Transition**: “With stable math + trustworthy numerics + GPU power, we can simulate exotic spacetimes.”

### Wormholes (research payoff)

- **Must land**: traversable wormholes require exotic negative-energy support (hypothetical).
- **Must land**: remove support → collapse → phantom bounce → gravitational waves.
- **Closing**: “We heard black holes merge in 2015. Tomorrow, we might catch a wormhole collapsing.”
...
**Must land:** Same GPU revolution that made ChatGPT possible also made cutting-edge GR simulations affordable.
...
**Must land:** Without NR, gravitational-wave astronomy is blind. The timeline showed decades of stuck theory — this plot shows *why that mattered*.
...
**Must land:** Without this differential geometry, the field equation could not have been written. The next slides unpack that toolkit (metric → Christoffel → Riemann → Einstein tensor).

**Show:** three animated vignettes — sphere/geodesics (Gauss), warped manifold with \(ds\) (Riemann), parallel transport (connection) — plus the heritage timeline ending at Einstein 1915.
...
**Show:** top = flat table — drag the probe, every ruler stays the same, \(ds^2 = dx^2 + dy^2\). Bottom = curved space — drag the mass, rulers stretch near \(M\), \(ds^2 = g_{\mu\nu}\,dx^\mu dx^\nu\), live \(ds\) readout changes.
...
**Must land:** AI boom mass-produces exactly this hardware; NR rides the same wave for free.
...
## Common confusions (quick fixes you can say out loud)

- **“Is gravity a force in GR?”**  
  Not in the fundamental description. In GR, free-fall is inertial motion; paths curve because geometry is curved.

- **“Are coordinates real?”**  
  Coordinates are addresses. The metric \(g_{\mu\nu}\) is what converts address differences into physical measurements (\(ds\)).

- **“What are constraints, practically?”**  
  They’re “this slice is a legal universe” checks. Good formulations keep violations from growing; CCZ4 actively damps them.

- **“Why do gauge choices matter if they’re not physics?”**  
  Because bad coordinates can crash a simulation even if the physics is fine. Gauge is choosing a camera, not changing the movie.

- **“What does curvature *do* physically?”**  
  Tidal effects: nearby free-fallers converge/diverge (geodesic deviation). That’s the measurable signature of curvature.
